...
class SearchPhrasesForm(Form):
    ...
    phrases = HiddenField('phrases', validators=[InputRequired(message='You have to enter at least one keyword to continue')])
...
class RegistrationForm(Form):
    ...
    password = PasswordField('New Password', validators=[InputRequired(message='Password field cannot be left blank'), EqualTo('confirm', message='Passwords must match')])
    confirm = PasswordField('Repeat Password',  validators=[InputRequired(message='Confirm Password field cannot be left blank')])
    ...
    gender = SelectField('gender', choices=[('female', 'Female'), ('male', 'Male')],  validators=[InputRequired(message='Gender field cannot be left blank')])


class LoginForm(Form):
    email = EmailField('Email', validators=[Email(message='Invalid Email format'), InputRequired(message='Email field cannot be left blank'), Length(1, 64)])
